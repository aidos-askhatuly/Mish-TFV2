...
class SemiHash():
    ...
    def bernoulliSample(x, do_det):
       """
        ...
        """
       train_fn = lambda: tf.cast(tf.random.uniform(tf.shape(x)) < x, tf.float32)
       eval_fn = lambda: tf.ones_like(x) * 0.5
       mus = tf.cond(do_det, eval_fn, train_fn)
       return tf.math.ceil(x - mus)
    ...
    def encoder(self, docbow):
        ...
        doc_layer = tf.keras.layers.Dense(self.args["layersize"], name="encode_layer0", activation=tf.nn.relu)(docbow)

        for i in range(1, self.args["layers"]):
            doc_layer = tf.keras.layers.Dense(int(self.args["layersize"]/i), name="encode_layer" + str(i),activation=tf.nn.relu)(doc_layer)
        ...
        sampling_vector = tf.keras.layers.Dense(self.args["bits"], name="last_encode", activation=tf.nn.sigmoid)(doc_layer)
    ...
    def get_doc_bow(self, doc_idx):
        ...
        doc_bow = tf.transpose(tf.sparse.sparse_dense_matmul(self.data_tensor, doc_idx_onehot))
        return doc_bow
    ...
    def get_most_similar_idx(self, memory, item):
        ...
        STH_num = tf.random.shuffle(STH_num) # random_shuffle only supports shuffling first dimension
    ...
    def get_memory_updates(self, doc_hashcode, emb_dtype, doc_STH, doc_idx, label, batchsize = None):
        ...
        mem_indices_expanded = tf.expand_dims(mem_indices, axis=1)
        hashcode_embedding_memory_update = tf.tensor_scatter_nd_update(self.hashcode_embedding_memory, mem_indices_expanded, tf.cast(doc_hashcode, emb_dtype))
        update_mem_indices = mem_indices.assign( (mem_indices + self.args["batchsize"]) % self.memsize )
        
        #hashcode_embedding_memory_update = tf.tensor_scatter_nd_update(self.hashcode_embedding_memory, td.expand_dims(mem_indices,1), tf.cast(doc_hashcode, emb_dtype))
        
        STHcodes_memory_update = tf.tensor_scatter_nd_update(self.STHcodes_memory, mem_indices_expanded, tf.cast(doc_STH, emb_dtype))
        
        doc_idx_memory_update = tf.tensor_scatter_nd_update(self.doc_idx_memory, mem_indices_expanded, doc_idx)
        label_memory_update = tf.tensor_scatter_nd_update(self.label_memory, mem_indices_expanded, label)
        all_updates = [hashcode_embedding_memory_update, STHcodes_memory_update, doc_idx_memory_update, label_memory_update]
        return all_updates, mem_indices_expanded, update_mem_indices
    ...
    def make_network(self, bow_sparse_matrix):
        ...
        self.importance_emb_matrix = tf.Variable(tf.random.uniform(shape=[self.args["bowlen"]], minval=0.1, maxval=1),
                        trainable=True, name="importance_embedding")

        if self.args['embedding_size']>0:
            self.word_emb_matrix = tf.Variable(tf.random.uniform(shape=[self.args["bowlen"], self.args["embedding_size"]], minval=-1, maxval=1),
                                                 trainable=True, name="word_embedding")
        else:
            self.word_emb_matrix = tf.Variable(tf.random.uniform(shape=[self.args["bowlen"], self.args["bits"]], minval=-1, maxval=1),
                                                 trainable=True, name="word_embedding")
        ...
        # Define Optimzier
        optimizer = tf.keras.optimizers.Adam(learning_rate=self.args["lr"], name="AdamOptimizer")
        ...
        # Further loss logic (bit_balance_loss, prob_pair_loss, etc.)
        ...
        trainable_vars = [self.importance_emb_matrix, self.word_emb_matrix, self.softmax_bias]
        gradients = tape.gradient(total_loss, trainable_vars)
       
       # Check for None gradients and remove them
        gradients_and_vars = [(grad, var) for grad, var in zip(gradients, trainable_vars) if grad is not None]
        
	#Apply the gradients using the optimizer
        optimizer.apply_gradients(gradients_and_vars)
        
        # Ensure doc_idx is a 2-D tensor with shape [40, 1]
        doc_idx_expanded = tf.expand_dims(doc_idx, axis=1)  # Shape: [40, 1]
        # Cast doc_hashcode_det to the appropriate data type
        
	 #Perform the scatter update
        emb_update = tf.tensor_scatter_nd_update(self.hashcode_embedding, doc_idx_expanded, tf.cast(doc_hashcode_det, emb_dtype))
        # Update embedding - old
        #emb_update = tf.tensor_scatter_nd_update(self.hashcode_embedding, doc_idx, tf.cast(doc_hashcode_det, emb_dtype))
        ...
        return optimizer, loss, emb_update, doc_loss, mem_updates, update_mem_indices, supervised_doc_loss, rank_loss, label_nbr_loss, \
