...
def generator(maxsize, handle, batchsize, record_paths, is_test, bowlen, labellen):
    def extract_fn(data_record):
        ...
        '''
        features = {
            'doc_bow_data': tf.io.VarLenFeature(tf.float32),
            'doc_bow_indices': tf.io.VarLenFeature(tf.int64),
            'doc_idx': tf.io.FixedLenFeature([1], tf.int64),
            'STHcode': tf.io.FixedLenFeature([64], tf.int64),
            'label': tf.io.FixedLenFeature([labellen], tf.int64)
        }
    ...
    #generic_iter = tf.data.Iterator.from_structure(output_t, output_s)
    specific_handle = next(iterator)

    return specific_handle, iterator
