...
def extract_vectors_labels(dist_opposite, dist_same, top_k_prec, handle, specific_handle, num_samples, batch_placeholder, is_training, sigma_anneal_vae,
    ...
    while not done:
        ...
        if mem_updates is None:
            ...
            top_k_prec_val = top_k_prec
            lossvals = loss
            loss_doc_only_vals = loss_doc_only
            sup_losses_doc_vals = super_doc_only
            rank_losses_doc_vals = rank_loss
            nbr_losses_doc_vals = nbr_loss
            bit_balance_loss_vals = bit_balance_loss

            # Handle memory updates if necessary
            for update in mem_updates[0]:
                update()

#            If mem_updates[1] is an operation, call it directly
            mem_updates[1]()

        losses += lossvals.numpy().tolist()
        losses_doc += loss_doc_only_vals.numpy().tolist()

        sup_losses_doc += sup_losses_doc_vals.numpy().tolist()
        rank_losses_doc += rank_losses_doc_vals.numpy().tolist()
        nbr_losses_doc += nbr_losses_doc_vals.numpy().tolist()
        top_k_prec_vals += top_k_prec_val.numpy().tolist()

        dist_opposite_list += dist_opposite_val.numpy().tolist()
        dist_same_list += dist_same_val.numpy().tolist()
    ...
    extracted_hashcodes = tf.convert_to_tensor(indices, dtype=tf.int32)
    ...
    with open(savename + "/run_log", "a") as file_handle:
        while running:
            ...
            #val_top_k_sims, val_top_k_prec, val_opposite, val_same, lossval = model_computation_fn(training_handle, args["batchsize"], True, vae_val, problem_pair_val, val_rotate_order)
             # Directly assign the required values within the loop

            val_top_k_sims = top_k_sims
            val_top_k_prec = top_k_prec
            val_opposite = dist_opposite
            val_same = dist_same
            lossval = loss
            valid_pairs_problem_pair_r = valid_pairs_problem_pair
            valid_pairs_top_k_pair_r = valid_pairs_top_k_pair

            # Optionally, perform updates for memory indices
            for update in update_mem_indices:
                update  # Assuming it's an operation

            losses+= lossval.numpy().tolist()
            train_count += 1
            vae_val = max(vae_val - vae_val_reduction, 0)
            top_k_prec_list += val_top_k_prec.numpy().tolist()
